...
disp_gt = double(imread('test_imgs/art_big.png'));%depth
gray_gt_ = im2double(rgb2gray(imread('test_imgs/view1.png')));%rgb
...
fprintf('%d,%d\n',size(ours));
csvwrite("test_imgs/ours.csv", ours);
...
weights = zeros(M,N);
weights(ours > 0) = 1;
csvwrite("test_imgs/weights.csv", weights);
