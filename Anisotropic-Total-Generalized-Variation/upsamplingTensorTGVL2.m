...
function [ u ] = upsamplingTensorTGVL2( u_init, d, w, gray, tensor_ab, tgv_alpha, l, maxits, check, tol, verbose )
    ...
    alpha0 = tgv_alpha(1);
    alpha1 = tgv_alpha(2);
    fprintf('size(gray) %d,%d alpha0 %d alpha1 %d\n',size(gray),alpha0,alpha1);
    ...
    fprintf('size(a) %d,%d \n',size(a)),
    csvwrite("test_imgs/a.csv", a);
    csvwrite("test_imgs/b.csv", b);
    csvwrite("test_imgs/c.csv", c);
    csvwrite("test_imgs/weights.csv", w);
